<div class="w-full pb-56">
    ...
    <div class="">
        <div id="accordion-color" data-accordion="collapse" data-active-classes="bg-yellow-300 text-slate-900" class="rounded-2xl border border-slate-200/[.10] overflow-hidden">
            @foreach ($faqs as $faq)
                <h2 id="accordion-color-heading-{{ $faq->id }}" class="group">
                    <button type="button" class="text-slate-200 group-hover:text-slate-900 border-b border-slate-200/[.10] flex items-center justify-between w-full p-6 rtl:text-right gap-3 hover:bg-yellow-300 bg-slate-200/[.03] duration-300 ease-in-out" data-accordion-target="#accordion-color-body-{{ $faq->id }}" aria-expanded="true" aria-controls="accordion-color-body-{{ $faq->id }}">
                ...
                <div id="accordion-color-body-{{ $faq->id }}" class="hidden bg-slate-200/[.08]" aria-labelledby="accordion-color-heading-{{ $faq->id }}">
